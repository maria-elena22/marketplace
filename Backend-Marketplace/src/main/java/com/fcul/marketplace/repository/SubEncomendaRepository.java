...
public interface SubEncomendaRepository extends JpaRepository<SubEncomenda, Integer> {
    ...
    @Query("SELECT s FROM SubEncomenda s " +
            "WHERE (:idFornecedor IS NULL OR s.fornecedor.idUtilizador = :idFornecedor) " +
    ...
    List<SubEncomenda> findAllOpt(Integer idFornecedor, Double precoMin, Double precoMax, Date dataMin, Date dataMax, EstadoEncomenda estadoEncomenda, Pageable pageable);
